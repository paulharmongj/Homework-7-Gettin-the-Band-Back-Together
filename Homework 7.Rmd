...
ar<-as.data.frame(cbind(seq(1,380,by=1),rep(12,109)))
ma<-as.data.frame(cbind(seq(1,380,by=1),rep(12,109)))
colnames(ar)<-c("index","noise")
Val<-CGO$Value
base<-gls(Val~YearF,method="ML",correlation=corAR1())
base.ma <- gls(Val~YearF,method="ML",correlation=corARMA(p=1,q=1))
for(i in 1:num.sims) {
  ar[,2]<-arima.sim(n=380,model=list(ar=c(.6)),sd=summary(base)$sigma)
  ma[,2]<-arima.sim(n=380,model=list(ar=c(.6)),sd=summary(base.ma)$sigma)
  model.ma.gls<-gls(noise~index,data=ar,correlation=corARMA(p=1,q=1))
