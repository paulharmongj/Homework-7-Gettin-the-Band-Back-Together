...
The AIC value for the GAM is given in the output below as -210.76. Whereas the previous AIC values that were used 
...
for(i in 1:num.sims) {
  ...
  model.ma.gls<-gls(noise~index,data=ma,correlation=corARMA(p=1,q=1))
  model.gls<-gls(noise~index,data=ar,correlation=corAR1())
  pvalue.nt<-summary(model.gls)$tTable[2,4]
  pvalue.mat<-summary(model.ma.gls)$tTable[2,4]
  happened.ma[i]<-pvalue.mat<.05
  happened[i]<-pvalue.nt<.05
}
sim.gls<-sum(happened==TRUE)/num.sims
sim.gls
