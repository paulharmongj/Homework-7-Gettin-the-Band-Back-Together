...
m9<-lm(CGO$Value~YearF2)
fits9<-fitted(m9)

#m10 - quad trend, seasonal means
m10<-lm(CGO$Value~YearF2+MonthF)
fits10<-fitted(m10)

#m11 - quad trend, single order harm
m11<-lm(CGO$Value~YearF2+harmonic(CGOts,m=1))
fits11<-fitted(m11)

#m12 - quad trend, 5th order harm
m12<-lm(CGO$Value~YearF2+harmonic(CGOts,m=5))
fits12<-fitted(m12)
...
for(i in 1:12) {
  changeinAIC[i]<-alltheAICs[i]-min(alltheAICs$AIC)
}
...
happened.lm.ar<-logical(length=num.sims)
happened.lm.ma<-logical(length=num.sims)
ar<-as.data.frame(cbind(seq(1,109,by=1),rep(12,109)))
ma<-as.data.frame(cbind(seq(1,109,by=1),rep(12,109)))
colnames(ar)<-c("index","noise")
colnames(ma)<-c("index","noise")

for(i in 1:num.sims) {
  ar[,2]<-arima.sim(n=109,model=list(ar=c(.6)),sd=sqrt(0.0006664659))
  ma[,2]<-arima.sim(n=109,model=list(ar=c(.6)),sd=sqrt(0.0006664659))
  model.ma.gls<-gls(noise~index,data=ma,correlation=corARMA(p=0,q=1))
  model.ar.gls<-gls(noise~index,data=ar,correlation=corARMA(p=1,q=0))
  model.lm.ma<-lm(noise~index,data=ma)
  model.lm.ar<-lm(noise~index,data=ar)
  pvalue.art<-summary(model.ar.gls)$tTable[2,4]
  pvalue.mat<-summary(model.ma.gls)$tTable[2,4]
  pvalue.lm.ma<-summary(model.lm.ma)$coefficient[2,4]
  pvalue.lm.ar<-summary(model.lm.ar)$coefficient[2,4]
  happened.ma[i]<-pvalue.mat<.05
  happened.ar[i]<-pvalue.art<.05
  happened.lm.ma[i]<-pvalue.lm.ma<.05
  happened.lm.ar[i]<-pvalue.lm.ar<.05
}
sim.gls.ar<-sum(happened.ar==TRUE)/num.sims
sim.gls.ar
sim.lm.ar<-sum(happened.lm.ar==TRUE)/num.sims
sim.lm.ar
sim.gls.ma<-sum(happened.ma==TRUE)/num.sims
sim.gls.ma
sim.lm.ma<-sum(happened.lm.ma==TRUE)/num.sims
sim.lm.ma
```
